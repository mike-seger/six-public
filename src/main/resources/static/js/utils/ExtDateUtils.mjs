...
import { localDate, isoDate } from './DateUtils.mjs'

function isWeekDay(date) {
	return !(date.getDay() === 6 || date.getDay() === 0)
}

export function plusExtraDays(isoDateStr, days, onlyWeekdays) {
	let isBankHoliday = onlyWeekdays === true
...
export function plusSwissWorkingDays(isoDateStr, days) {
	const result = plusExtraDays(isoDateStr, days, isSwissBankHoliDay)
	return result
}

export function getPrevPeriod(date, period) {
	date = localDate(date)
